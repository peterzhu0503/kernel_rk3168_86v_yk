...
extern int __sramdata charge_power_off;
static void __sramfunc rk30_charge_poweroff(void)
{
	int value;
	
	while(1)
	{
        value = readl(RK30_GPIO0_BASE + 0x50);
        sram_udelay(2000);
        if(!(value&0x10))  //power key detect pin, low when  pressed
	{
			break;
        }
    
        if(!(value&0x400)) //charge detect pin ,high when not charge
	{
			charge_power_off = 0;
			break;
        }
