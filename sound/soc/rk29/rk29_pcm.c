...
static void rockchip_pcm_enqueue(struct snd_pcm_substream *substream)
{
	...
	prtd->dma_pos = pos;
			...
			{
				int a = 0;
				unsigned char *pos_t = substream->runtime->dma_area;
				for(a=0; a< prtd->dma_period*prtd->dma_loaded;a++){
					if(*pos_t) {
						tmp_data[data_p] = *pos_t;
						data_p ++;
						if(data_p > 1024*1024*2)	
						{
							printk(KERN_DEBUG "data buffer over \n");	
							pcm_data_debug = 0;
							break;
						}	
					}
...
static int proc_pcm_show(struct seq_file *s, void *v)
{
	int a;
	printk("i2s pcm data debug = %d\n",pcm_data_debug);
	printk("pcm_tmp_buf data_p =%d\n",data_p);

		for(a=0;a<=data_p;a++)
		{
			printk(KERN_DEBUG "%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
			tmp_data[a],tmp_data[a++],tmp_data[a++],tmp_data[a++],tmp_data[a++],
			tmp_data[a++],tmp_data[a++],tmp_data[a++],tmp_data[a++],tmp_data[a++]);			
		}
		printk("\n");
data_p = 0;
memset(tmp_data,0,1024*1024*4);
	return 0;
}

static ssize_t proc_pcm_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    pcm_data_debug = value;
	printk("i2s pcm data debug = %d\n",pcm_data_debug);
	printk("pcm_tmp_buf data_p =%d  (max=1024*1024*4)\n",data_p);

	return buf_size;
}

static int proc_pcm_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_pcm_show, NULL);
}

static const struct file_operations proc_pcm_fops = {
	.open		= proc_pcm_open,
	.read		= seq_read,
	.write      = proc_pcm_write,	
	.llseek		= seq_lseek,
...
{
	proc_create("i2s_pcm_debug", 0, NULL, &proc_pcm_fops);
	return 0;
}
