...
struct rk610_codec_priv {
	...
	int call_enable;	
	int headset_status;	
	struct delayed_work call_delayed_work;
};
...
void codec_set_spk(bool on)
{
	...
	if(!rk610_codec)
		return;
	if(on)
		rk610_codec->hdmi_ndet = DISABLE;
	else
		rk610_codec->hdmi_ndet = ENABLE;	
	DBG(" codec_set_spk hdmi_ndet = %d\n", rk610_codec->hdmi_ndet);
	if((rk610_codec->hdmi_ndet == DISABLE) && (rk610_codec->headset_status == HP_OUT))
...
static int rk610_codec_write_incall(struct snd_soc_codec *codec, unsigned int reg,
	unsigned int value);
static int rk610_codec_update_bits(struct snd_soc_codec *codec, unsigned short reg,
				unsigned int mask, unsigned int value)
{
	...
	ret = rk610_codec_read(codec, reg);
	if (ret < 0)
		return ret;
	...
	if (change) {
		...
		rk610_codec_write_incall(codec,reg,new);
	}
...
void call_set_spk(int on)
{
	...
	if(!rk610_codec)
		return;
        rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R17,0x40,0x40);  //AOL
        rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R18,0x40,0x40);
	switch(on)
	{
	case 0:
		...
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R13, 0x08, 0);	
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R14, 0x08, 0);
		rk610_codec->call_enable = 0;	
#ifdef USE_MIC_IN
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R12, 0x80, 0x80);	 
#else	
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R12, 0x80, 0);	 
#endif
	        if(rk610_codec->headset_status == HP_IN)
	                spk_ctrl_fun(GPIO_LOW);
        	else
                	spk_ctrl_fun(GPIO_HIGH);
		rk610_codec_write(rk610_codec_codec,ACCELCODEC_R15, 0xC1);
		 rk610_codec_write(rk610_codec_codec,ACCELCODEC_R17, gVolReg|ASC_OUTPUT_ACTIVE|ASC_CROSSZERO_EN); 
                rk610_codec_write(rk610_codec_codec,ACCELCODEC_R18, gVolReg|ASC_OUTPUT_ACTIVE|ASC_CROSSZERO_EN); 
        	rk610_codec_write(rk610_codec_codec,ACCELCODEC_R04, ASC_INT_ACTIVE_L|ASC_INT_ACTIVE_R|ASC_SIDETONE_L_OFF|ASC_SIDETONE_R_OFF);
                rk610_codec_write(rk610_codec_codec,ACCELCODEC_R19, 0x7F);  //AOM
                msleep(300);
                #if OUT_CAPLESS
        	rk610_codec_write(rk610_codec_codec,ACCELCODEC_R1F, 0x09|ASC_PDMIXM_ENABLE);
        	#else
        	rk610_codec_write(rk610_codec_codec,ACCELCODEC_R1F, 0x09|ASC_PDMIXM_ENABLE|ASC_PDPAM_ENABLE);
		#endif
	...
	case 1:
		...
		rk610_codec_write(rk610_codec_codec,ACCELCODEC_R15, 0x01);
		rk610_codec_write_incall(rk610_codec_codec,ACCELCODEC_R0E, 0x00);
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R12, 0x80, 0);	 
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R13, 0x08, 0x08);	
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R14, 0x08, 0x08);
		 schedule_delayed_work(&rk610_codec->call_delayed_work, msecs_to_jiffies(800));
	...
	case 2:
		...
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R13, 0x08, 0x08);	
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R14, 0x08, 0x08);
		 schedule_delayed_work(&rk610_codec->call_delayed_work, msecs_to_jiffies(1000));
	...
	case 3:
		...
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R13, 0x08, 0x08);	
		rk610_codec_update_bits(rk610_codec_codec,ACCELCODEC_R14, 0x08, 0x08);	
		 schedule_delayed_work(&rk610_codec->call_delayed_work, msecs_to_jiffies(500));
...
static void call_delayedwork_fun(struct work_struct *work)
{
	...
	rk610_codec_update_bits(codec,ACCELCODEC_R17,0x40,0);  //AOL
        rk610_codec_update_bits(codec,ACCELCODEC_R18,0x40,0);
...
void rk2928_codec_set_spk(bool on)
{
	...
	printk("%s: headset %s %s PA bias_level=%d\n",__FUNCTION__,on?"in":"out",on?"disable":"enable",rk610_codec_codec->dapm.bias_level);
	if(on) {
		if(rk610_codec->spk_ctrl_io){
			gpio_set_value(rk610_codec->spk_ctrl_io, GPIO_LOW);
			level = gpio_get_value(rk610_codec->spk_ctrl_io);
			printk("%s:spk_ctrl_io = %d ,level= %d\n",__func__,rk610_codec->spk_ctrl_io,level);
		}
	}
	else {
		
		level = gpio_get_value(rk610_codec->spk_ctrl_io);
		printk("%s:dapm.bias_level = %d ,level= %d\n",__func__,rk610_codec_codec->dapm.bias_level,level);

		if(rk610_codec_codec->dapm.bias_level == SND_SOC_BIAS_STANDBY 
		|| rk610_codec_codec->dapm.bias_level == SND_SOC_BIAS_OFF){
			return;
		}
		if(rk610_codec->spk_ctrl_io)
			gpio_set_value(rk610_codec->spk_ctrl_io, GPIO_HIGH);
		printk("%s:spk_ctrl_io level= %d\n",__func__,gpio_get_value(rk610_codec->spk_ctrl_io));
	}
}
#endif
...
static int rk610_codec_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&rk610_codec->call_delayed_work, call_delayedwork_fun);	
	if(rk610_codec->spk_ctrl_io)
	{
	    ...
	    gpio_direction_output(rk610_codec->spk_ctrl_io, GPIO_HIGH);
	    gpio_set_value(rk610_codec->spk_ctrl_io, GPIO_HIGH);	
	}
